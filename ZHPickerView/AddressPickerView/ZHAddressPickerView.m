...
@interface ZHAddressPickerView ()<UIPickerViewDelegate,UIPickerViewDataSource>
{
...
// 选中后的回调
@property (nonatomic, copy) ZHAddressResultBlock resultBlock;

@end

@implementation ZHAddressPickerView
...
+ (void)showAddressPickerWithDefaultSelected:(NSArray *)defaultSelectedArr isAutoSelect:(BOOL)isAutoSelect resultBlock:(ZHAddressResultBlock)resultBlock {
    ZHAddressPickerView *addressPickerView = [[ZHAddressPickerView alloc] initWithDefaultSelected:defaultSelectedArr isAutoSelect:isAutoSelect resultBlock:resultBlock];
    [addressPickerView showWithAnimation:YES];
}

#pragma mark - 初始化地址选择器
- (instancetype)initWithDefaultSelected:(NSArray *)defaultSelectedArr isAutoSelect:(BOOL)isAutoSelect resultBlock:(ZHAddressResultBlock)resultBlock {
    if (self = [super init]) {
        // 默认选中
...
#pragma mark - 获取地址数据
- (void)loadData {
    NSString *filePath = [[NSBundle mainBundle] pathForResource:@"ZHCity" ofType:@"plist"];
    NSArray *arrData = [NSArray arrayWithContentsOfFile:filePath];
    for (NSDictionary *dic in arrData) {
        // 此处用 YYModel 进行解析
        ZHProvinceModel *proviceModel = [ZHProvinceModel yy_modelWithDictionary:dic];
...
- (NSInteger)pickerView:(UIPickerView *)pickerView numberOfRowsInComponent:(NSInteger)component {
    ZHProvinceModel *provinceModel = self.addressModelArr[rowOfProvince];
    ZHCityModel *cityModel = provinceModel.city[rowOfCity];
    if (component == 0) {
        //返回省个数
...
- (nullable NSString *)pickerView:(UIPickerView *)pickerView titleForRow:(NSInteger)row forComponent:(NSInteger)component {
    ...
    if (component == 0) {//省
        ZHProvinceModel *provinceModel = self.addressModelArr[row];
        showTitleValue = provinceModel.name;
    }
    if (component == 1) {//市
        ZHProvinceModel *provinceModel = self.addressModelArr[rowOfProvince];
        ZHCityModel *cityModel = provinceModel.city[row];
        showTitleValue = cityModel.name;
    }
    if (component == 2) {//区
        ZHProvinceModel *provinceModel = self.addressModelArr[rowOfProvince];
        ZHCityModel *cityModel = provinceModel.city[rowOfCity];
        ZHTownModel *townModel = cityModel.town[row];
    ...
    if (rowOfProvince < self.addressModelArr.count) {
        ...
        if (rowOfCity < provinceModel.city.count) {
            ...
            if (rowOfTown < cityModel.town.count) {
                ZHTownModel *townModel = cityModel.town[rowOfTown];
...
- (void)scrollToRow:(NSInteger)firstRow secondRow:(NSInteger)secondRow thirdRow:(NSInteger)thirdRow {
    if (firstRow < self.addressModelArr.count) {
        ...
        ZHProvinceModel *provinceModel = self.addressModelArr[firstRow];
        if (secondRow < provinceModel.city.count) {
            rowOfCity = secondRow;
            [self.pickerView reloadComponent:1];
            ZHCityModel *cityModel = provinceModel.city[secondRow];
